<x-redsys-radio-selector :id="$card->id" :name="'mode'" :label="__(config('redsys.translationsPrefix') . 'payWith') . ' ' . $card->alias"
                         ...
                         :selected="$selected">
    <a onclick="onTokenizedCardPressed('{{ $card->id }}')" class="token-card relative cursor-pointer block w-full h-12 flex-row justify-center text-center items-center rounded bg-black">
